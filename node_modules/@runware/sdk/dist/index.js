...
// src/_version.ts
var SDK_VERSION = "1.6.7";

// src/_schemas-version.ts
var SCHEMAS_VERSION = "20260709212957";
...
var isNodeJS = () => typeof process !== "undefined" && typeof process.versions?.node === "string";
...
var computeReconnectDelay = (retryDelay, attempt, randomFn = Math.random) => {
  const base2 = retryDelay * Math.pow(2, attempt - 1);
  const capped = Math.min(base2, RECONNECT_DELAY_CAP_MS);
...
var excludeNonNative = (models2) => models2.filter((model) => !NON_NATIVE_STATUSES.has(model.status ?? ""));
  ...
  try {
    ...
    const ackPromises = tasks.map(async (task) => new Promise((resolve, reject) => {
      ...
      ackResolvers.set(task.taskUUID, (result) => {
        const timer2 = ackTimers.get(task.taskUUID);
        if (timer2) {
          clearTimeout(timer2);
          ackTimers.delete(task.taskUUID);
        }
...
//# debugId=EA953F0D359D479464756E2164756E21
